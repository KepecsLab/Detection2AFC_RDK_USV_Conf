...
if isempty(fieldnames(TaskParameters))
    ...
    TaskParameters.GUI.size = 14;                   % size of dots (pixels)
    ...
    TaskParameters.GUI.speed = 80;       %degrees/second, we must multiply by 2 because we are halving the framerate
    TaskParameters.GUI.nDots = 300;
    TaskParameters.GUI.lifetime = 20;  %lifetime of each dot (frames)
    TaskParameters.GUI.StimSignalLen = .2;
    TaskParameters.GUIPanels.Dots= {'finite_play', 'coherence_type', 'color', 'size', 'centerX','centerY', 'apertureX', 'apertureY' 'speed', 'lifetime', 'nDots', 'StimSignalLen'}; 
...
switch TaskParameters.GUI.StimulusType
    ...
    case 3
        ...
        coherence = BpodSystem.Data.Custom.Coherence(a);
        if dots{a}.direction == 0
            coherence = 0;
        end

        [dirs, dx, dy] =...
            compute_dirs(num_trials, dots{a}.nDots, coherence,...
            dots{a}.direction, dots{a}.speed, 60);
        [rdirs, drx, dry] = compute_dirs(num_trials, dots{a}.nDots, 0,...
                            dots{a}.direction, dots{a}.speed,60);
        dots{a}.rdirs = rdirs;
        dots{a}.drx = drx;
        dots{a}.dry = dry;
        ...
        BpodSystem.Data.Custom.display = display;
        BpodSystem.Data.Custom.CenterRewarded(a)= BpodSystem.Data.Custom.correct_side(a) == 0;
        BpodSystem.Data.Custom.LeftRewarded(a)= BpodSystem.Data.Custom.correct_side(a) < 0;
        BpodSystem.Data.Custom.RightRewarded(a)= BpodSystem.Data.Custom.correct_side(a) > 0;
...
while RunSession
   ...
   % InitiatePsychtoolbox(iTrial);
    ...
    %%Prep avisoft - one instance for recorder and one for speaker. Program
    %%needs to be open already

    %BpodSystem.Data.Avisoft1=ddeinit('RECORDER','main');
    %BpodSystem.Data.Avisoft2=ddeinit('RECORDER2','main');
    %% Run Trial
    %ddeexec(BpodSystem.Data.Avisoft1,'start')
    RawEvents = RunStateMatrix;
    %ddeexec(BpodSystem.Data.Avisoft1,'stop')
